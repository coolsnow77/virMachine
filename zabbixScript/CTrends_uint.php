 ...
 * @package API
 */
class CTrends_uint extends CZBXAPI {

	protected $tableName = 'trends_uint';
	...
	public function get($options = array()) {
		...
		$sqlParts = array(
			'select'	=> array('trends_uint' => 'h.itemid'),
			'from'		=> array(),
			'where'		=> array(),
			'group'		=> array(),
			'order'		=> array(),
		...
		$sqlParts['from']['trends_uint'] = $tableName.' h';
		...
		if (is_array($options['filter'])) {
			$this->dbFilter($sqlParts['from']['trends_uint'], $options, $sqlParts);
		}

		// search
		if (is_array($options['search'])) {
			zbx_db_search($sqlParts['from']['trends_uint'], $options, $sqlParts);
		}

		// output
		if ($options['output'] == API_OUTPUT_EXTEND) {
			unset($sqlParts['select']['clock']);
			$sqlParts['select']['trends_uint'] = 'h.*';
		}
	...
	# cut it 
	protected function applyQuerySortOptions_old($tableName, $tableAlias, array $options, array $sqlParts) {
		$isIdFieldUsed = false;
